...
test('identical popup controls preserve their existing DOM nodes', () => {
    ...
    const actionBar = shell.querySelector('.hoshi-reader-popup-action-bar');
    const sasayakiBar = shell.querySelector('.hoshi-reader-popup-sasayaki-bar');

    scene.host.renderStack({
        popups: [{ ...payload, frame: { ...payload.frame } }],
    });

    assert.strictEqual(shell.querySelector('.hoshi-reader-popup-action-bar'), actionBar);
    assert.strictEqual(shell.querySelector('.hoshi-reader-popup-sasayaki-bar'), sasayakiBar);
});

test('root highlight preserves identical DOM nodes and redraws changed rectangles', () => {
    const highlight = {
        popupId: 'root',
        pending: false,
        eInkMode: true,
        ...
        rects: [{ x: 20, y: 24, width: 30, height: 16 }],
    };
    const scene = renderRootHighlightScene(highlight);
    const highlightLayer = scene.layer.querySelector('.hoshi-reader-selection-highlight-layer');
    const originalNode = highlightLayer.children[0];

    scene.host.renderStack({
        popups: [rootPopupPayload()],
        rootHighlight: { ...highlight, rects: highlight.rects.map((rect) => ({ ...rect })) },
    });
    assert.strictEqual(highlightLayer.children[0], originalNode);

    scene.host.renderStack({
        popups: [rootPopupPayload()],
        rootHighlight: { ...highlight, rects: [{ ...highlight.rects[0], x: 28 }] },
    });
    assert.notStrictEqual(highlightLayer.children[0], originalNode);
    assert.equal(highlightLayer.children[0].style.left, '28px');
});

test('sasayaki highlight preserves identical DOM nodes and redraws changed rectangles', () => {
    const scene = popupHost();
    ...
    scene.document.documentElement.style['--hoshi-eink-line-color'] = '#000';
    const highlight = {
        eInkMode: true,
        verticalWriting: false,
        rects: [{ x: 12, y: 20, width: 40, height: 18 }],
    };
    scene.host.renderSasayakiHighlight(highlight);
    const layer = scene.document.getElementById('hoshi-reader-popup-layer')
        .querySelector('.hoshi-reader-sasayaki-highlight-layer');
    const originalNode = layer.children[0];

    scene.host.renderSasayakiHighlight({
        ...highlight,
        rects: highlight.rects.map((rect) => ({ ...rect })),
    });
    assert.strictEqual(layer.children[0], originalNode);

    scene.host.renderSasayakiHighlight({
        ...highlight,
        rects: [{ ...highlight.rects[0], width: 48 }],
    });
    assert.notStrictEqual(layer.children[0], originalNode);
    assert.equal(layer.children[0].style.width, '48px');
});
