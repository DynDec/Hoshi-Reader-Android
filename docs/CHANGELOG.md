...
- Add a Reader Appearance Layout toggle to remove vertical borders, zeroing the
  reader's effective vertical padding and using 10dp top and 6dp bottom safe areas
  while hiding border shortcuts and bottom progress, placing the toggle with the
  other Layout toggles above Avoid Page Break, and restoring the user's normal
...
- Add an optional Reader Behavior setting that hides the bookshelf progress
  bar and percentage without changing progress inside the Reader.
